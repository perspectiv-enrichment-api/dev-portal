...
export default function APIKeysPage() {
  ...
  const handleCreateKey = async () => {
    ...
    try {
      const token = await authStore.token();
      const { key } = await keysApi.create(token, newKeyLabel.trim());
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
        ...
        <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>Enter a label for your new API key</DialogDescription>
            </DialogHeader>
            {!generatedKey ? (
              ...
              <CardContent className="pt-6">
                <div className="space-y-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-3">
                        <h3 className="text-lg font-semibold text-foreground">{apiKey.label}</h3>
                        <span
                  ...
                  <div className="bg-background/50 border border-border rounded-lg p-4 flex items-center justify-between gap-4">
                    <span className="font-mono text-sm text-muted-foreground">{apiKey.key_prefix}••••••••••••••••</span>
                    <Button
                      variant="ghost"
                      size="sm"
      ...
      <Card className="bg-card/50 backdrop-blur border-border">
        <CardHeader>
          ...
          <CardDescription>Example request to the merchant enrichment endpoint</CardDescription>
        </CardHeader>
        <CardContent>
